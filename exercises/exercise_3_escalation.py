    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If your `confidence` is below 0.60, populate `escalation_questions` "
                "with 2–4 specific, context-rich questions that a human reviewer must "
                "answer before you can finalize the review. Each question MUST reference "
                "the concrete file path and the section/symbol in the diff (e.g. "
                "`auth/login.py:hash_password` — why MD5?). Avoid generic questions."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    qa_block = "\n".join(f"Q: {q}\nA: {ans}" for q, ans in qa_pairs.items())

    llm = get_llm().with_structured_output(PRAnalysis)
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = llm.invoke([
            {"role": "system", "content": (
                "You previously reviewed a PR with low confidence and asked the "
                "human reviewer clarifying questions. Their answers are below. "
                "Produce a refined structured review that incorporates their "
                "context. Your new confidence should usually be HIGHER than the "
                "original (since the open questions are now answered)."
            )},
            {"role": "user", "content": (
                f"Title: {state['pr_title']}\n\n"
                f"--- Original analysis ---\n"
                f"Summary: {initial.summary}\n"
                f"Risks: {', '.join(initial.risk_factors)}\n"
